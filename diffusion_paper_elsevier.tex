...
Consider a polygonal cell with $N_V$ vertices. The polygon needs not 
...
$\sum_{i=1}^{N_V} \alpha_{i} = 1$. Point $c$ i a weighted average
...
%
\begin{equation}
  b_j(\br) = t_j(\br) + \alpha_{c,j} t_c(\br) , \qquad j=1,..,N_V$ ,
\end{equation}
%
where $t_j(\br)$ is the standard linear functions defined on the two
